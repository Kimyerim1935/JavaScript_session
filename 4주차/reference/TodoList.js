import { isValidConstructor, isValidTarget, isValidParameter } from './utils.js'
...
function TodoList({ $target, initialState, onClickTodo, onDeleteTodo }) {
  ...
  this.render = function () {
    const content = this.state.reduce((acc, cur) => {
      const { _id, content, isCompleted } = cur

      return (
        acc +
        `<li data-id=${_id} class="${isCompleted ? 'todo completed' : 'todo'}">
            ${content}
